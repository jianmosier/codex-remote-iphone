...
- `setup`: run `npm run setup` to verify or download the project-local `cloudflared` binary before first start. The command should print staged progress and heartbeat lines while downloading; if the user reports it looks stuck, ask for the latest visible setup output.
