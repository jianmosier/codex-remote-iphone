...
- `setup`: run `npm run setup` to verify or download the project-local `cloudflared` binary before first start. If download is needed, the command should explicitly print the download source, install location, project-local scope, staged progress, and heartbeat lines; if the user reports it looks stuck, ask for the latest visible setup output.
